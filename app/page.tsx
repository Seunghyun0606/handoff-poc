...
import type { AnalysisResult, CategoryStatus, Gap } from "@/lib/types";
  ...
  return (
    <main className="min-h-screen">
      <section className="mx-auto max-w-6xl px-5 pb-10 pt-16 md:pt-24">
        ...
        <div className="mx-auto mt-10 max-w-4xl rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/50 md:p-7">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <label htmlFor="handoff" className="font-bold text-slate-900">인수인계 문서</label>
            <label className="cursor-pointer rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50">.txt / .md 업로드<input className="hidden" type="file" accept=".txt,.md,text/plain,text/markdown" onChange={onFile} /></label>
          </div>
          <textarea id="handoff" value={text} onChange={(e) => setText(e.target.value)} placeholder="인수인계 문서를 붙여넣어 주세요..." className="min-h-64 w-full resize-y rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-7 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-50" />
          <div className="mt-4 flex flex-wrap gap-2">
            {SAMPLES.map((sample) => (
              <button key={sample.id} onClick={() => { setText(sample.text); setResult(null); }} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:border-indigo-300 hover:text-indigo-700">{sample.label} <span className="text-slate-400">({sample.expected})</span></button>
            ))}
          </div>
          {error && <p className="mt-3 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}
          <button disabled={loading || text.trim().length < 20} onClick={analyze} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 font-bold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-40">{loading ? "AI가 인수자의 입장에서 검토 중..." : "AI 분석하기"}</button>
        ...
        <section id="result" className="border-t border-slate-200 bg-white/70 py-12">
          <div className="mx-auto max-w-6xl px-5">
            ...
            <div className="mt-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
              ...
              {tab === "questions" && <ol className="mt-5 space-y-3">{result.questions.map((question, index) => <li key={question} className="flex gap-3 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-indigo-100 text-xs font-black text-indigo-700">{index + 1}</span><span>{question}</span></li>)}</ol>}

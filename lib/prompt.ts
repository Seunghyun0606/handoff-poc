...
- questions는 PARTIAL 또는 MISSING으로 판정한 카테고리에 대해서만 만든다. CLEAR 카테고리에는 질문하지 않는다.
- 각 카테고리당 최대 1개 질문만 만들고, 실제 인수 시 가치가 높은 질문을 우선해 전체 최대 8개로 제한한다.
- 각 question에는 반드시 해당 category_id를 함께 반환한다.
